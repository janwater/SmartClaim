...
parentDir = 'C:/Users/E154709/Desktop/Mohawk/Claims/OnlineVersion'
os.chdir(parentDir)

# load functions from python file
from blagging import BlaggingClassifier

from functions import _pull_hana_odbc
...
from functions import _concat_duplicate_columns

# load models
from models import label_encoder

# load constants
from constants import INPUT_TAGS, OUTPUT_TAGS
...
def main(finput, imode = 1):
    ...
    # <codecell>
        ...
        #save accummulated output @3-5-2018
        pre_date = datetime.date.today()-datetime.timedelta(1)
        fpredate = _create_filename(outputPath, pre_date.strftime("%Y-%m-%d"), 'accum_', affix) 
        faccum = _create_filename(outputPath, now.strftime("%Y-%m-%d"), 'accum_', affix) 
        
        # read pre_date  result data
        with open (fpredate, 'r') as csvfile:
            df_predate = pd.read_csv(csvfile) 
        
        # drop duplicate columns
        df_predate = _concat_duplicate_columns(df_predate)
        pred_op = _concat_duplicate_columns(pred_op)
        
        df_predate = df_predate.set_index(['CLAIM_NUM'])
        pred_op = pred_op.set_index(['CLAIM_NUM'])
        #df_predate = df_predate.drop(['Unnamed: 0'], axis=1)
                
        #combine two DFs and drop duplicate rows
        df_accum = pd.concat([df_predate, pred_op])
        df_accum = df_accum.reset_index(drop=False)
        df_accum = df_accum.drop_duplicates(subset=['CLAIM_NUM'])         
        df_accum = df_accum[OUTPUT_TAGS]
                
        with open (faccum, 'w') as csvfile:
            df_accum.to_csv(csvfile) 
            
        fsave_accu = _create_filename(savePath, curFname, 'accum_', affix)
        with open (fsave_accu, 'w') as csvfile:
            df_accum.to_csv(csvfile)
